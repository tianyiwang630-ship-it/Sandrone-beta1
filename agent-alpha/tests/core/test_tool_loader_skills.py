...
def test_tool_loader_loads_agent_home_skills_dir():
    tmp_dir = make_test_dir("tool-loader")
    try:
        project_root = tmp_dir / "project"
        skill_dir = project_root / "home" / ".agents" / "skills" / "agent-reach"
...
""",
        ...
        with patch.object(ToolLoader, "_load_mcp_tools", lambda self: None), patch.object(
            ...
            result = loader.execute_tool("load_skill", {"name": "agent-reach"})
            assert result == '<skill name="agent-reach">\nUse agent reach.\n</skill>'
    finally:
        cleanup_test_dir(tmp_dir)
